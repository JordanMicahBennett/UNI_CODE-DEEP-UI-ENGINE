...
public class UNICODE_BUI_FIELD extends JPanel 
{
    //attributes
        ...
        private UNICODE_MenuPanel buiActionDock = null;
	...
	public void reInitializeField ( String value, boolean forwardNavigationEnquiry )
	{
		if ( getStreamEmptyEnquiry ( value ) ) //if folder is not empty.
		{
			...
			new UNICODE_MessageBoxWindow ( "Directory is empty!", 0.9f, backgroundColour, buttonOutlineColour, labelBackgroundColour, labelForegroundColour, true, "data/images/all/message box/okay/","rr", buttonWidth, buttonHeight, arcWidth, arcHeight, axisDisplacement, lastButtonChopValue  );
	}
			...
			if ( !parameterBubble.getParameterBubbleEnableEnquiry ( ) )
			{
				for ( int i = 0; i < maxBuis; i ++ )
				{
					...
					if ( getViewModeAnswer ( ).equals ( "off" ) )
					{
						...
						{
							buiActionDock.audioPlayer.playAudio ( "enter.wav" );
							buiActionDock.setVisible ( true );
							...
							if ( buiBody.representsDirectory ( buiBody.getEntityStream ( ) ) )
								buiActionDock.setBackground ( descriptionModeBuiFolderColour );
							else
								buiActionDock.setBackground ( descriptionModeBuiFileColour );
    ...
    //dock animation functions
        ...
        private class nodeEntityMenuAnimationListener implements ActionListener
        {
            ...
            {
                buiActionDock.performAxisAnimation ( 70, 180, 2.4f, 0.756f, 0.3f, "clockwise", nodeEntityMenuAnimationTimer );
                repaint ( );
            }
        }    
        public UNICODE_MenuPanel getBuiActionDock ( )
        {
            return buiActionDock;
        }
        ...
        //mutators
        public void setBuiActionDock ( UNICODE_MenuPanel _buiActionDock )
        {
            buiActionDock = _buiActionDock;
        }
	...
	public Thread [ ] getDescriptorCollectionSelectedActionThreads ( )
	{
		...
		(
		   new Runnable ( )
		   {
			   public void run ( )
			   {
					new UNICODE_MessageBoxWindow ( "file(s) successfully copied!", 0.9f, backgroundColour, buttonOutlineColour, labelBackgroundColour, labelForegroundColour , true, "data/images/all/message box/okay/","rr", 20, 20, 10, 10, 0, 8 );    
		...
		(
			new Runnable ( )
			{
				public void run ( )
				{
					new UNICODE_MessageBoxWindow ( "file(s) successfully cut!", 0.9f, backgroundColour, buttonOutlineColour, labelBackgroundColour, labelForegroundColour , true, "data/images/all/message box/okay/","rr", 20, 20, 10, 10, 0, 8 );    
					dynamicHiglighter.reset ( );
					parameterBubble.reset ( runnableFieldInstance );
				}
			}
					...
					(
						new Runnable ( )
						{
							public void run ( )
							{
								...
								new UNICODE_MessageBoxWindow ( "file(s) successfully deleted!", 0.9f, backgroundColour, buttonOutlineColour, labelBackgroundColour, labelForegroundColour , true, "data/images/all/message box/okay/","rr", 20, 20, 10, 10, 0, 8 );   
							}
						}
					);
					
					new UNICODE_MessageBoxWindow ( fileDeletionThread, "are you certain?", 0.9f, backgroundColour, buttonOutlineColour, labelBackgroundColour, labelForegroundColour , true, "data/images/all/message box/okay - threaded/","rr", 20, 20, 10, 10, 0, 8 );   
					dynamicHiglighter.reset ( );
					parameterBubble.reset ( runnableFieldInstance );
				}
			}
